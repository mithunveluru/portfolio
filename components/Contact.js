const Contact = () => {
  const [formState, setFormState] = React.useState({
    name: '', email: '', subject: '', message: '',
  });
  const [status, setStatus] = React.useState('idle');

  const handleChange = (e) => {
    setFormState(prev => ({ ...prev, [e.target.name]: e.target.value }));
  };

  const handleSubmit = (e) => {
    e.preventDefault();
    setStatus('sending');

    const sub  = encodeURIComponent(
      formState.subject || `Hey Mithun — reaching out from your portfolio`
    );
    const body = encodeURIComponent(
      `Hi Mithun,\n\nName: ${formState.name}\nEmail: ${formState.email}\n\n${formState.message}\n`
    );
    window.location.href = `mailto:mithunveluru7@gmail.com?subject=${sub}&body=${body}`;

    setTimeout(() => {
      setStatus('sent');
      setFormState({ name: '', email: '', subject: '', message: '' });
    }, 900);
  };

  const links = [
    {
      label:   'Email',
      value:   'mithunveluru7@gmail.com',
      href:    'mailto:mithunveluru7@gmail.com',
      meta:    'Fastest response',
      icon: (
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none"
          stroke="currentColor" strokeWidth="2"
          strokeLinecap="round" strokeLinejoin="round">
          <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1
            0-2-.9-2-2V6c0-1.1.9-2 2-2z"/>
          <polyline points="22,6 12,13 2,6"/>
        </svg>
      ),
    },
    {
      label:   'GitHub',
      value:   'github.com/mithunveluru',
      href:    'https://github.com/mithunveluru',
      meta:    'See what I\'m building',
      icon: (
        <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
          <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18
            6.839 9.504.5.092.682-.217.682-.483
            0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343
            -.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608
            1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088
            2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113
            -4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446
            -1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0 1
            12 6.844a9.59 9.59 0 0 1 2.504.337c1.909-1.296 2.747-1.027
            2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595
            1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92
            .678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688
            .482A10.02 10.02 0 0 0 22 12.017C22 6.484 17.522 2 12 2z"/>
        </svg>
      ),
    },
    {
      label:   'LinkedIn',
      value:   'linkedin.com/in/mithunveluru',
      href:    'https://linkedin.com/in/mithunveluru',
      meta:    'Let\'s connect professionally',
      icon: (
        <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
          <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037
            -1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046
            c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286z
            M5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1
            2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771
            C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451
            C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
        </svg>
      ),
    },
  ];

  const reasons = [
    'SDE Internship · Summer / Fall 2026',
    'ML or data engineering roles',
    'Open source collaboration',
    'Just want to talk systems',
  ];

  return (
    <section id="contact" className="section">
      <div className="container">

        {/* ── Header ── */}
        <div className="contact-header">
          <span className="section-label">05 / contact</span>
          <h2 className="section-title">
            Let's work together.
          </h2>
          <p className="contact-subtitle">
            I'm actively looking for internships where I can
            ship real systems alongside engineers who care about craft.
            If that sounds like your team — let's talk.
          </p>
        </div>

        <div className="contact-layout">

          {/* ── LEFT ── */}
          <div className="contact-left">

            {/* Availability card */}
            <div className="contact-availability">
              <div className="contact-availability__indicator">
                <span className="contact-availability__dot" />
                <span className="contact-availability__text">
                  Open to opportunities · Summer 2026
                </span>
              </div>
              <p className="contact-availability__sub">
                VIT Vellore · India · Open to remote globally
              </p>
            </div>

            {/* Good fit block */}
            <div className="contact-fit">
              <span className="contact-fit__label">Good fit if you need</span>
              <ul className="contact-fit__list">
                {reasons.map((r, i) => (
                  <li key={i} className="contact-fit__item">
                    <svg width="12" height="12" viewBox="0 0 24 24"
                      fill="none" stroke="currentColor"
                      strokeWidth="3" strokeLinecap="round"
                      strokeLinejoin="round">
                      <polyline points="20 6 9 17 4 12"/>
                    </svg>
                    {r}
                  </li>
                ))}
              </ul>
            </div>

            {/* Links */}
            <div className="contact-links">
              {links.map(({ label, value, href, meta, icon }) => (
                <a
                  key={label}
                  href={href}
                  target={label !== 'Email' ? '_blank' : undefined}
                  rel="noopener noreferrer"
                  className="contact-link"
                >
                  <div className="contact-link__icon">{icon}</div>
                  <div className="contact-link__content">
                    <span className="contact-link__label">{label}</span>
                    <span className="contact-link__value">{value}</span>
                  </div>
                  <span className="contact-link__meta">{meta}</span>
                  <svg
                    className="contact-link__arrow"
                    width="14" height="14" viewBox="0 0 24 24"
                    fill="none" stroke="currentColor"
                    strokeWidth="2.5" strokeLinecap="round"
                    strokeLinejoin="round">
                    <line x1="7" y1="17" x2="17" y2="7"/>
                    <polyline points="7 7 17 7 17 17"/>
                  </svg>
                </a>
              ))}
            </div>

            {/* Response note */}
            <p className="contact-note">
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none"
                stroke="currentColor" strokeWidth="2"
                strokeLinecap="round" strokeLinejoin="round">
                <circle cx="12" cy="12" r="10"/>
                <polyline points="12 6 12 12 16 14"/>
              </svg>
              Usually responds within 24 hours
            </p>

          </div>

          {/* ── RIGHT — Form ── */}
          <div className="contact-right">
            <div className="contact-form-wrap">

              <div className="contact-form-header">
                <span className="contact-form-header__title">
                  Send a message
                </span>
                <span className="contact-form-header__sub">
                  Goes straight to my inbox — no middleware
                </span>
              </div>

              {status === 'sent' ? (
                <div className="contact-sent">
                  <div className="contact-sent__icon">
                    <svg width="22" height="22" viewBox="0 0 24 24"
                      fill="none" stroke="currentColor"
                      strokeWidth="2.5" strokeLinecap="round"
                      strokeLinejoin="round">
                      <polyline points="20 6 9 17 4 12"/>
                    </svg>
                  </div>
                  <h4 className="contact-sent__title">On its way.</h4>
                  <p className="contact-sent__sub">
                    Your mail client should be open. I'll reply within 24 hours.
                  </p>
                  <button
                    className="contact-sent__reset"
                    onClick={() => setStatus('idle')}
                  >
                    Send another →
                  </button>
                </div>
              ) : (
                <form className="contact-form" onSubmit={handleSubmit}
                  noValidate>

                  <div className="form-row">
                    <div className="form-field">
                      <label className="form-label" htmlFor="name">
                        Name
                      </label>
                      <input
                        id="name" name="name" type="text"
                        className="form-input"
                        placeholder="Your name"
                        value={formState.name}
                        onChange={handleChange}
                        required autoComplete="off"
                      />
                    </div>
                    <div className="form-field">
                      <label className="form-label" htmlFor="email">
                        Email
                      </label>
                      <input
                        id="email" name="email" type="email"
                        className="form-input"
                        placeholder="your@email.com"
                        value={formState.email}
                        onChange={handleChange}
                        required autoComplete="off"
                      />
                    </div>
                  </div>

                  <div className="form-field">
                    <label className="form-label" htmlFor="subject">
                      Subject
                      <span className="form-label__opt">optional</span>
                    </label>
                    <input
                      id="subject" name="subject" type="text"
                      className="form-input"
                      placeholder="Internship · Collab · Just saying hi"
                      value={formState.subject}
                      onChange={handleChange}
                      autoComplete="off"
                    />
                  </div>

                  <div className="form-field">
                    <label className="form-label" htmlFor="message">
                      Message
                    </label>
                    <textarea
                      id="message" name="message"
                      className="form-input form-textarea"
                      placeholder="What are you working on? What do you need?"
                      value={formState.message}
                      onChange={handleChange}
                      required rows={5}
                    />
                  </div>

                  <button
                    type="submit"
                    className={`form-submit ${status === 'sending' ? 'form-submit--sending' : ''}`}
                    disabled={status === 'sending'}
                  >
                    {status === 'sending' ? (
                      <>
                        <span className="form-submit__spinner" />
                        Opening mail client...
                      </>
                    ) : (
                      <>
                        Send Message
                        <svg width="14" height="14" viewBox="0 0 24 24"
                          fill="none" stroke="currentColor"
                          strokeWidth="2.5" strokeLinecap="round"
                          strokeLinejoin="round">
                          <line x1="22" y1="2" x2="11" y2="13"/>
                          <polygon points="22 2 15 22 11 13 2 9 22 2"/>
                        </svg>
                      </>
                    )}
                  </button>

                  <p className="form-disclaimer">
                    Opens your default mail client with everything pre-filled.
                  </p>

                </form>
              )}

            </div>
          </div>

        </div>
      </div>
    </section>
  );
};